datDir <- './'
can <- "vilon.online"
##metaDir <- paste0('tmp/',can,'/intermediate_files/')
...
if (cfgTable$online.clinical==TRUE){
    file=paste0(datDir, paste(dataTypes, collapse='.'),".T.comPatients.",can,".rda")
    load(file,Data <- new.env())
    Data <- as.list(Data)

    results <- as.list(dir(datDir,ignore.case=F,pattern="*.Rda"))
    metTypes <- sub(".*[.]","",sub("[.]Rda","",results,ignore.case=T));
    names(results) <- metTypes
    for(met in metTypes){
        load(results[[met]])
        results[[met]] <- stats
    }
    ...
    adjusted.res <- results
                                        # adjust p-values per tool
    for(met in metTypes){
        dat <- dd[dd$met == met, ]
        dat$q <- p.adjust(as.numeric(dat$p), method='BH')
        adjusted.res[[met]] <- dat
    }
    dd <- do.call(rbind,adjusted.res)
    ...
    tabSav <- dd[,tabSel]
    write.table(tabSav[order(-tabSav$prod),], paste0("./results/",can,'.all.tab'), sep = "\t", row.names=F, quote=F)
    ...
    combs <- data.frame(met=factor(ddd$met,levels=c('ViLoN','SNF', 'LRAcluster', 'clinical')),
                        mod=factor(ddd$model,levels=c('int')),cn=ddd$cn)
    ...
    dev.copy2pdf(file=paste0("./results/",paste(dataTypes,collapse="."),"-",paste(mets,collapse="."),"-",can,'.pdf'))
