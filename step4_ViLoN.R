...
for (d in dataTypes) {
    ...
    files <- complete
    #patients <- 5
    #sel <- sample(length(d1Files), patients)
    sel <- 1:length(files) # in case we want to make a smaller network (e.g., for testing)
...
minC <- 2
maxC <- Inf # to allow for automated selection
